        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '健康知识名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.jiankangName" placeholder="健康知识名称" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('jiankangLiuyan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/chongwujiankangguwen/upload/jiankangLiuyanMuBan.xls"
                        >批量导入健康知识评论数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('jiankangLiuyan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="jiankangLiuyanUploadSuccess"
                                :on-error="jiankangLiuyanUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('jiankangLiuyan','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入健康知识评论数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('jiankangLiuyan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "jiankangLiuyan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="jiankangLiuyanText"
                                      header-align="center"
                                      label="评论内容">
                        <template slot-scope="scope">
                            <span v-if="scope.row.jiankangLiuyanText != null &&scope.row.jiankangLiuyanText.length>10">
                                {{scope.row.jiankangLiuyanText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.jiankangLiuyanText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('jiankangLiuyan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('jiankangLiuyan','修改') && sessionTable=='guwen' && userId==scope.row.jiankangGuwenId" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>
                            <el-button v-if="isAuth('jiankangLiuyan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            json_fields: {
                //级联表字段
                     '健康知识名称': 'jiankangName',
                     '健康知识编号': 'jiankangUuidNumber',
                     '健康知识照片': 'jiankangPhoto',
                     '健康知识类型': 'jiankangTypes',
                     '健康知识视频': 'jiankangVideo',
                     '健康知识文件': 'jiankangFile',
                     '用户姓名': 'yonghuName',
                     '用户手机号': 'yonghuPhone',
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'jiankang_liuyan',//当前表表名,
                        sumColum : 'jiankang_liuyan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'jiankang_liuyan_name',//分组字符串字段
                        // types : 'jiankang_liuyan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "jiankang_liuyan_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['jiankangLiuyanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "jiankangLiuyan/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            jiankangLiuyanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "jiankangLiuyan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入健康知识评论数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
            ...
            // 导入功能上传文件失败后调用导入方法
            jiankangLiuyanUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "jiankangLiuyan/update",
                        method: "post",
                        data: {
                            id:id,
//                            jiankangLiuyanTypes:jiankangLiuyanTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
